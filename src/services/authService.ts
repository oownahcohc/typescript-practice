...
class AuthService {
    ...
    public async SignUp (signupDto: SignupDTO) {
        ...
        /** @Error1 필수 요청 값 누락 */
        if(!email || !password || !nickname) return responseError.NULL_VALUE;
        /** @Error2 이메일 형식 오류 */
        if (!isEmail(email)) return responseError.WRONG_EMAIL_CONVENTION;
        /** @Error3 비밀번호 형식 오류 */
        if (!passwordValidator(password)) return responseError.WRONG_PASSWORD_CONVENTION;

        try {
            const existUser = await this.userModel.findOne({ where: { email }});
            /** @Error4 이미 존재하는 유저 */
            if (existUser) return responseError.USER_ALREADY_EXIST;
            ...
            /** @Error5 유저 생성 실패 */
            if (!newUser) return responseError.FAIL_SIGNUP;
    ...
    public async LogIn (loginDto: LoginDTO) {
        ...
        /** @Error1 필수 요청 값 누락 */
        if(!email || !password) return responseError.NULL_VALUE;

        try {
            const isUser = await this.userModel.findOne({ where: { email } });
            /** @Error2 존재하지 않는 유저 */
            if (!isUser) return responseError.NON_EXISTENT_USER;
            const isMatch = await bcrypt.compare(password, isUser.password);
            /** @Error3 비밀번호가 일치하지 않음 */
            if (!isMatch) return responseError.PW_NOT_CORRECT;
    ...
    public async ReissueToken (reissueTokenDto: TokenDTO) {
        ...
        /** @Error1 필수 요청 값 누락 */
        if (!accessToken || !refreshToken) return responseError.NULL_VALUE;
        ...
        try {
            ...
            if (refreshTokenDecoded === TOKEN_EXPIRED || refreshTokenDecoded === TOKEN_INVALID) {
                return responseError.TOKEN_EXPIRES;
            }
